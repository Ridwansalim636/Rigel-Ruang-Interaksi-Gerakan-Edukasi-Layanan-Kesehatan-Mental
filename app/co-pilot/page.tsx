...
export default function CoPilotPage() {
  ...
  const sendMessageToAPI = async (textToSend: string) => {
    ...
    try {
      // 2. Format ulang seluruh percakapan ke format yang dimengerti Gemini API
      const apiHistory = updatedMessages.map((msg) => ({
        role: msg.sender === 'user' ? 'user' : 'model',
        parts: [{ text: msg.text }],
      }));

      // 3. Kirim data riwayat lengkap ke backend
      ...
      // 4. Tampilkan jawaban cerdas dari Gemini
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0B132B] via-[#1C2541] to-[#3A506B] text-white font-sans flex flex-col justify-between">
      ...
      <div className="border-b border-white/10 backdrop-blur-md bg-[#0B132B]/60 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <div className="bg-cyan-500/20 text-cyan-400 p-2 rounded-lg border border-cyan-500/30 w-10 h-10 flex items-center justify-center font-bold">
            🤖
          </div>
          <div>
        ...
        <a 
          href="/" 
          className="text-xs text-slate-400 hover:text-cyan-400 border border-white/10 px-3 py-1.5 rounded-md backdrop-blur-sm transition-all"
        >
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-md transition-all ${
                    msg.sender === 'user'
                      ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-br-none shadow-cyan-500/10'
      ...
      <div className="p-6 max-w-3xl w-full mx-auto sticky bottom-0 bg-gradient-to-t from-[#3A506B] via-[#3A506B]/90 to-transparent space-y-3">
        ...
        {/* Tombol Pertanyaan Cepat (Quick Templates) */}
        <div className="flex flex-wrap gap-2 justify-center">
          {quickTemplates.map((template, idx) => (
            <button
              key={idx}
              type="button"
              onClick={() => sendMessageToAPI(template)}
              className="text-xs bg-white/5 hover:bg-cyan-500/20 border border-white/10 hover:border-cyan-500/40 px-3 py-1.5 rounded-full transition-all text-slate-300 hover:text-cyan-400 active:scale-95"
            >
              {template}
            </button>
          ))}
        ...
        <form onSubmit={handleFormSubmit} className="relative flex items-center">
          <input
            ...
            placeholder="Ketik curhatanmu di sini secara privat..."
            className="w-full bg-[#0B132B]/80 border border-white/10 focus:border-cyan-400 rounded-xl py-3.5 pl-4 pr-14 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-400/20 transition-all backdrop-blur-md shadow-inner"
          />
          <button
            type="submit"
            className="absolute right-2 bg-cyan-500 hover:bg-cyan-400 text-white p-2 rounded-lg font-bold transition-all shadow-lg shadow-cyan-500/20 active:scale-95 flex items-center justify-center w-9 h-9"
          >
            🚀
          </button>
        </form>
        <p className="text-center text-[10px] text-slate-400/60">
          🔒 Obrolan ini bersifat privat dan dienkripsi secara aman.
        </p>
      </div>
